...
export default function Filter() {
  return (
    <div className="flex justify-between items-center w-full px-20 mt-4 max-[800px]:px-10">
      <div className="flex flex-row justify-start items-center gap-x-4 w-10/12 h-20 overflow-x-auto max-[1235px]:w-9/12 max-[800px]:w-full">
        {filterData.map((item, i) => (
          <FilterDataItem item={item} key={i} />
        ))}
      </div>
      <div className="rounded-full border-2 border-black w-8 h-8 flex items-center justify-center max-[800px]:hidden cursor-pointer">
